  ...
  if (eventType === "user.created" || eventType === "user.updated") {
    ...
    if (existingUser.length === 0) {
      ...
      await db
        .update(users)
        .set({
          email: email_addresses[0]?.email_address || existingUser[0].email,
          username: username || existingUser[0].username,
          displayName: first_name && last_name ? `${first_name} ${last_name}` : existingUser[0].displayName,
          avatarUrl: image_url || existingUser[0].avatarUrl,
          updatedAt: new Date(),
        })
